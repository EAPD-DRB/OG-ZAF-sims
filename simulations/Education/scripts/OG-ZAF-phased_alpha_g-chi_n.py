...
def main():
    ...
    save_dir = os.path.join(CUR_DIR, "OG-ZAF-phased_alpha_g-chi_n")
    ...
    # The factors are chosen to gradually increase alpha_G over 3 years.
    # The final period has the largest increase of 7.1%.
    factors = [1.002, 1.035, 1.071]
    for i, v in enumerate(factors):
        p2.alpha_G[i] *= v
    p2.alpha_G[len(factors) :] *= factors[-1]

    # decrease the first 15 values in the matrix chi_n by 5 (ages 20-35)
    p2.chi_n[:15] -= 5

    # Adjust eta parameter for the first 3 income groups and normalize
    # p2.eta[:, :, 3] *= 1.0001
    # sum_eta = np.sum(p2.eta[1, :, :])
    # p2.eta /= sum_eta
